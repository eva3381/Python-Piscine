def events_processor(event_list: list):
    """
    Yields game events one by one from a list.
    This approach ensures constant memory usage by streaming data 
    instead of duplicating the entire collection.
    """
...
def finbonacci_generator():
    ...
    Maintains internal state to calculate the next value only when requested.
    """
    a = 0
    b = 1
    while True:
...
def prime_generator():
    """
    Iteratively calculates and yields prime numbers.
    Uses trial division to validate each candidate before yielding.
    """
    a = 2
    while True:
        prime = True
